...
class UserProfileView(generics.UpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        if ser.is_valid():
            ser.save()
            return Response({"message": "Updated User successfully"}, status=HTTP_200_OK)
...
class SignUpAPIView(APIView):
    ...
    def post(self, request):
        ...
        # create a user 
        user_obj = ser.save()
        return Response({'user': UserSerializer(user_obj).data}, status=HTTP_201_CREATED)
...
class UserFCMAPI(APIView):
    ...
    def post(self, request):
        ...
        if not fcm_token:
            return Response({"error": "FCM Token is a requored field"}, status=HTTP_400_BAD_REQUEST)
        user = request.user
